...
**Metric toggle buttons** (grade view) — horizontal row of pill buttons inside the sticky toolbar band. Inactive state is a neutral chip (border, body text); active state fills brand-purple with white text. Four real metrics are available on every grade view, always in the same order — Words correct per minute, High-frequency words, Nonsense word fluency, Letter naming fluency — followed by a fifth view-mode toggle, **Additional Growth**, which re-renders the table to the continuum view (see Additional Growth view below). WCPM is selected by default for every grade. Below 768px viewport, the buttons collapse into a `<select>` dropdown with the same order. `role="tablist"`; `aria-selected` reflects the active state.

**Additional Growth view** (grade view; selected via the 5th toggle option) — replaces the standard student table with a continuum-based row layout that translates each student's score into where they sit on the K-2 learning journey. Columns: Student (sticky), Teacher, Today's skill, Continuum band, Reading level, Gap closed, Gap remaining.

The **Continuum band** is a horizontal bar 18px tall, segmented by three brand-tinted background colors (K Skills 0-15 WCPM, 1st Skills 15-50, 2nd Skills 50-110) sitting under a status-colored solid growth fill (BOY → current), a diagonal slash zone (current → projected), three journey markers (open Start circle, filled Current dot, open Projected ring) all stroked in the status color, and a vertical grade-level benchmark tick at the student's grade WCPM target. The bar shares the WCPM x-axis used elsewhere.

**Reader-type buckets** drive the Reading level column (Started / Now / EOY): Pre-Reader (WCPM < 10), Word Reader (10-40), Fluent Reader (40-80), Multisyllabic Fluent Reader (80+). These thresholds are Acadience-style placeholders pending Ignite confirmation.

**Today's skill** maps the student's current WCPM to one of 13 partner-facing curriculum stages (e.g., "Silent-e words", "Long vowel teams", "Compound words, prefixes, suffixes") derived from the Instructional Scope and Sequence. The mapping is a prototype-only placeholder; production will read each student's actual lesson progress directly from Ignite's instructional system. Internal protocol names and abbreviations are intentionally omitted.

**Gap closed** is the months-of-growth proxy (`(current − BOY) / (target − BOY) × MT`) measured against the student's grade-level WCPM target. **Gap remaining** is the additional months of typical growth still needed for the student's projected EOY score to reach the grade target; reads "On grade" when projected ≥ target. Mastered students (BOY ≥ grade WCPM target — none under current placeholder data) collapse to a single "On grade level — no gap remaining" row with a green check icon.
