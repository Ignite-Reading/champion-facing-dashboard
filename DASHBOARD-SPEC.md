...
1. **District view** — cards for each school, each showing three aggregate charts (months of progress, oral reading fluency, high-frequency words) across all grades at that school. Schools are listed alphabetically.
...
**Estimate availability / graceful degradation.** Production year-end estimates likely won't exist until mid-year, so the charts must render without them. A global `ESTIMATES_AVAILABLE` flag (default `true`) gates every estimate-derived element; flip it to `false` to preview the no-estimate state. When estimates are absent: the projected-growth slash, the estimated-growth pill, the on-track/below status (colors + icons), the "% estimated to meet benchmark" stat, and the student-table Estimated-growth value all drop out; and the three summary cards fall back to current values ("Current [metric]", subtitle "to date"). The growth fill stays the on-track green (we are still showing growth, just no status), and the benchmark tick/pill stays (a fixed grade-level standard, known regardless of estimates).
...
**Summary metric cards** (`.metrics-row`, `.mc`) — auto-fit grid of three cards (`minmax(220px, 1fr)`). White surface, soft border, card shadow, 16-20px padding. Each card shows a 14px secondary-text label (with a `min-height: 2.7em` reservation so 1- and 2-line labels align across the three cards), a 32px weight-700 value, a 12px "by year-end" subtitle, and a green trending-up icon plus "+N since last week" trend line in `--ok-fg`. Every view shows the same three cards: **Current Months of Progress Estimate · Current Oral Reading Fluency Estimate · Current High-Frequency Words Estimate**. The value is unchanged — the year-end estimate from data to date — framed as a current estimate rather than a projection. Scope is aggregate across all grades at the district and school levels, and grade-specific at the grade view. The "since last week" trend is a deterministic placeholder derived from the value (the prototype has no real time-series history).
...
**Aggregate metric charts** (on district school cards and school grade cards) — three charts per card in a responsive grid (3-across desktop, 2-across at ≤1024px, 1-across at ≤640px). Each card shows the same three charts in the same order: Months of Progress, Oral Reading Fluency, High-Frequency Words.
...
7. **Three aggregate charts per card.** District and school cards show months of progress, oral reading fluency, and HFW side by side (3-across desktop, 2-across tablet, 1-across mobile), each aggregating real scores across all students in scope.
...
- Renamed "Trending to" / "Trending-to" to "Estimated growth" everywhere it surfaces (column header, legend, pills; the stat reads "estimated to meet benchmark").
...
- Renamed the "Months of Growth" metric to "Months of Progress" everywhere (chart title, summary card, pills).
- Changed the Months of Progress chart's stat to "+N average growth" (growth to date), matching the ORF and HFW charts, instead of a future year-end projection.

### Earlier
- See git history for the full prior sequence: the IRDL brand restyle (v7 app shell, sidebar, sticky bands, dark mode, accessibility), the WCPM→ORF and PA→NWF renames, the aggregate-chart redesign (months of progress / ORF / HFW), the mastery and not-yet-at-this-level row treatments, and the move to the Ignite Reading org repo (`champion-facing-dashboard`).
