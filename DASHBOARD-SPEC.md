...
Each chart shares a common shape: a 13px primary-text label above a 28px tall bar with rounded ends. The WCPM and HFW charts contain a status-colored solid fill from average BOY to average current, a diagonal slash pattern from average current to average projected, a white start dot with status-colored stroke at the BOY position, a vertical target tick at the target position, and pill labels overlaying the bar at the BOY / Current / Projected positions. The pills are small white chips with a tertiary border and primary-text numbers. When pills would collide horizontally, the lower-priority pill is hidden in this order: Start > Projected > Current. The target is not a pill on the bar — its value appears on the first line below the chart ("Target: N"), with the tick still marking its position on the bar. A second line reads "+N average growth · X% projected to meet target".
